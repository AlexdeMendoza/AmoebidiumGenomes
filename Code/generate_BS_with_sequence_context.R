...
#This uses FASTAs to extract the location and nucleotide context of each cytosine in the genome of: Amoebidium, methylated control (pUC19), and unmethylated control (lambda)
build_cg_loci_from_fasta <- function(ORGANISM_FASTA_PATH, width_of_context=5){
  MET <- read.fasta(MET_FASTA_PATH) #This is from the methylated control, pUC19, spiked into EMseq.
  UNMET <- read.fasta(UNMET_FASTA_PATH) #This is from the unmethylated control, lambda, spiked into EMseq.
  FASTA <- read.fasta(ORGANISM_FASTA_PATH)
  FASTA <-c(MET, UNMET, FASTA)
  distance_from_center <- (width_of_context-1)/2 #We define the distance around each cytosine that we want to include in our "context"
  positions_to_find <- seq(-distance_from_center, distance_from_center)
  #This loops through each contig in the assembly and finds the cytosines and their contexts.
...
#This function loads methylation data from a CGmap into the CG loci generated using the previous function.
#It allows multiple CGmaps to be loaded into one object describing multiple samples, if desired. That functionality is not used here.
...
#This function creates a BSseq object
...
#List of inputs:
ORGANISM_FASTA_PATH <- "Aapp_genome.fasta.gz"
MET_FASTA_PATH <- "pUC19.fasta.gz"
UNMET_FASTA_PATH <- "lambda.fasta.gz"
TREATMENTS <- "NA"
SAMPLES <- "NA"
CGMAP_PATH <- c("EMseq_Aapp_control.CGmap.gz") #CGmap generated from EMseq_Aapp_control.read1.fastq.gz using a combination of bs_seeker2 and cgmaptools
FASTA_INDEX_PATH <- "./inputs/genomes/Aapp_genome_pUC19_lambda.fasta.fai"
